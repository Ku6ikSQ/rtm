...
import type { CreateAlbumDto, CreateTrackInput } from '@/types/entities'
...
export function CreateAlbumPage() {
  ...
  const {
    ...
    defaultValues: { genreIds: [], releaseYear: new Date().getFullYear() },
  })
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormValues) => {
      ...
      const dto: CreateAlbumDto = {
        ...
        tracks: trackInputs.length > 0 ? trackInputs : undefined,
        links: links.filter((l) => l.platformId && l.url.trim()).map((l) => ({ platformId: l.platformId, url: l.url.trim() })),
      }
      const album = await albumService.create(dto)
      if (coverFile) await albumService.uploadCover(album.id, coverFile)
      return album
    },
    onSuccess: (album) => {
      queryClient.invalidateQueries({ queryKey: ['albums'] })
      toast.success('Альбом добавлен')
  ...
  function updateTrack(id: string, field: keyof TrackRow, value: string) {
  ...
  if (loadingArtists || loadingGenres || loadingPlatforms) return <PageSpinner />

  const rootGenres = genres.filter((g) => !g.parentId)

  return (
    <div className="mx-auto max-w-lg">
      <h1 className="mb-6 text-xl font-bold">Добавить альбом</h1>
      ...
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-5">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Длительность в формате <span className="font-mono">3:45</span> или секундах. Пустые треки игнорируются.
          </p>
        </div>
        ...
        <div className="flex gap-3 pt-1">
          ...
          >
            {mutation.isPending ? 'Сохраняем…' : 'Добавить альбом'}
          </button>
          <button
            type="button"
            onClick={() => navigate(-1)}
